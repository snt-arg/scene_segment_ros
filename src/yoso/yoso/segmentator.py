...
class YOSO(nn.Module):
    def __init__(self, cfg):
        ...
        self.sem_seg_postprocess_before_inference = (
            cfg.MODEL.YOSO.TEST.SEM_SEG_POSTPROCESSING_BEFORE_INFERENCE or cfg.MODEL.YOSO.TEST.PANOPTIC_ON or cfg.MODEL.YOSO.TEST.INSTANCE_ON)
        ...
        self.yoso_head = YOSOHead(
            cfg=cfg, num_stages=cfg.MODEL.YOSO.NUM_STAGES, criterion=criterion)

        self.register_buffer("pixel_mean", torch.Tensor(
            cfg.MODEL.PIXEL_MEAN).view(-1, 1, 1), False)
        self.register_buffer("pixel_std", torch.Tensor(
            cfg.MODEL.PIXEL_STD).view(-1, 1, 1), False)
        self.to(self.device)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            losses, cls_scores, mask_preds = self.yoso_head(
                neck_feats, targets)
            return losses
        else:
    ...
    def semantic_inference(self, mask_cls, mask_pred):
        ...
        semseg = semseg/0.25  # temperature scaling
    ...
    def instance_inference(self, mask_cls, mask_pred):
        ...
        labels = torch.arange(self.num_classes, device=self.device).unsqueeze(
            0).repeat(self.num_proposals, 1).flatten(0, 1)
        ...
        # torch.div(topk_indices, self.num_classes, rounding_mode='floor') #
