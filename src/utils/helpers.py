...
def getYOLOFilteredSegments(predictions, classes, image_shape):
    """
    Returns YOLO segmentation results filtered by the defined classes.

    Parameters
    -------
    predictions:
        The results generated by the YOLO model.
    classes: list
        The list of classes to be filtered.
    image_shape: tuple
        The original image shape as (H, W).
    ...
    if result.semantic_mask is None:
        ...
        filteredProbs = np.zeros((h, w, len(classes)), dtype=np.float32)
        return {
            "sem_seg": torch.from_numpy(filteredProbs).permute(2, 0, 1),
            "panoptic_seg": (torch.from_numpy(sem_seg).to(torch.int32), []),
        }, filteredProbs

    semanticMask = result.semantic_mask.data

    if isinstance(semanticMask, torch.Tensor):
        semanticMask = semanticMask.cpu().numpy()

    semanticMask = semanticMask.astype(np.int32)

    if semanticMask.shape != (h, w):
        semanticMask = cv2.resize(
            semanticMask,
            (w, h),
    ...
    for outputClassId, classGroup in enumerate(classes):
        classMask = np.isin(semanticMask, classGroup)

        if not np.any(classMask):
            continue
    ...
    filteredSegments = {
        "sem_seg": torch.from_numpy(filteredProbs).permute(2, 0, 1),
        "panoptic_seg": (
            torch.from_numpy(outputMask).to(torch.int32),
            segmentsInfo,
        ),
    }
