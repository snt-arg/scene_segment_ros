...
def probabilities2ROSMsg(
    probabilities: np.ndarray, timestamp, frame_id: str
) -> PointCloud2:
    """
    Converts probabilities to a ROS 2 PointCloud2 message

    Parameters
    -------
    probabilities: np.ndarray
        The probabilities of the semantic segmented image from the neural network as HxWxC
    timestamp: builtin_interfaces.msg.Time or rclpy.time.Time
        The timestamp should be the same as the image as that is when the observation was produced
    frame_id: str
        The frame id for the message
    """
    point_cloud_msg = PointCloud2()
    # ROS 2: timestamp can be builtin_interfaces.msg.Time or rclpy.time.Time
    if hasattr(timestamp, "to_msg"):
        point_cloud_msg.header.stamp = timestamp.to_msg()
    else:
    ...
    fields = [
        PointField(
            name="prob", offset=0, datatype=PointField.FLOAT32, count=num_classes
        )
    ]
