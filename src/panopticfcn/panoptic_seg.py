...
class PanopticFCN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.semantic_fpn = build_semanticfpn(
    ...
    def losses(self, pred_centers, pred_regions, pred_weights, encode_feat, gt_dict):
        ...
        thing_pred, _ = self.thing_generator(
            encode_feat, feat_shape, idx_feat_th, thing_num)
        stuff_pred, _ = self.stuff_generator(
            encode_feat, feat_shape, idx_feat_st, stuff_num)

        # for thing
        thing_gt_idx = [_gt[:, :thing_nums[_idx]]
                        for _idx, _gt in enumerate(gt_dict["index_mask"])]
        thing_gt_idx = torch.cat(thing_gt_idx, dim=1)
        thing_gt_idx = thing_gt_idx.reshape(-1).bool()
        thing_gt_num = int(thing_gt_idx.sum())
        thing_gt = [_gt[:, :thing_nums[_idx], ...]
                    for _idx, _gt in enumerate(gt_dict["inst"])]
        ...
        loss_thing = weighted_dice_loss(thing_pred, thing_gt,
                                        ...
                                        reduction="sum")
        # for stuff
        stuff_gt_idx = [_gt[:, :stuff_nums[_idx]]
                        for _idx, _gt in enumerate(gt_dict["sem_index"])]
        stuff_gt_idx = torch.cat(stuff_gt_idx, dim=1)
        stuff_gt_idx = stuff_gt_idx.reshape(-1).bool()
        stuff_gt_num = int(stuff_gt_idx.sum())
        stuff_gt = [_gt[:, :stuff_nums[_idx], ...]
                    for _idx, _gt in enumerate(gt_dict["sem_labels"])]
        ...
        loss = {}
        # position loss
        loss["loss_pos_th"] = self.pos_weight * \
            sum(loss_pos_ths) / max(thing_gt_num, 1)
        loss["loss_pos_st"] = self.pos_weight * \
            sum(loss_pos_sts) / max(feat_shape[0], 1)
        # segmentation loss
        loss["loss_seg_th"] = self.seg_weight * \
            loss_thing / max(thing_gt_num, 1)
        loss["loss_seg_st"] = self.seg_weight * \
            loss_stuff / max(stuff_gt_num, 1)
    ...
    def loss_single_level(self, pred_center, pred_region, pred_weights,
        ...
        guided_inst = F.interpolate(gt_inst, size=(
            feat_h, feat_w), mode='bilinear', align_corners=False)
        ...
        idx_feat_th = idx_feat_th.reshape(
            *weight_shape[:2], thing_num, self.weighted_num)
        # generate guided sem
    ...
    def inference_single_level(self, pred_center, pred_region, pred_weights, pool_size):
        ...
        top_num = min(center_shape[-2] *
                      center_shape[-1], self.center_top_num//2)
        ...
        score_st = (pred_region * pred_st_mask).reshape(1,
                                                        self.sem_classes, -1)
        score_st = (score_st.sum(dim=-1)
                    [:, class_st] / num_class_st).squeeze(0)
    ...
    def inference(self, batch_inputs, images, pred_centers, pred_regions, pred_weights, encode_feat):
        ...
        for img_idx, result_img in enumerate(results):
            ...
            pred_regions = [_pred[img_idx].unsqueeze(
                0) for _pred in pred_regions]
            pred_weights = [_pred[img_idx].unsqueeze(
                0) for _pred in pred_weights]
            pred_centers = [_pred[img_idx].unsqueeze(
                0) for _pred in pred_centers]
            pool_size = [3, 3, 3, 5, 5]
            ...
            if thing_num == 0:
                ...
                idx_feat_th = [
                    _feat for _feat in idx_feat_th if len(_feat) > 0]
            ...
            if stuff_num == 0:
                ...
                class_sts = [
                    _cate_sem for _cate_sem in class_sts if len(_cate_sem) > 0]
                idx_feat_st = [
                    _feat for _feat in idx_feat_st if len(_feat) > 0]
                score_sts = torch.cat(score_sts, dim=0)
                class_sts = torch.cat(class_sts, dim=0)
                idx_feat_st = torch.cat(idx_feat_st, dim=0)

            pred_thing, [class_ths, score_ths] = self.thing_generator(
                encode_feat, feat_shape, idx_feat_th, thing_num, class_ths, score_ths)
            pred_stuff, [class_sts, score_sts] = self.stuff_generator(
                encode_feat, feat_shape, idx_feat_st, stuff_num, class_sts, score_sts)
            ...
            pred_stuff = F.interpolate(
                pred_stuff, size=ori_shape, mode="bilinear", align_corners=False)[0]
            pred_sem_seg = torch.zeros(
                sem_classes, *pred_stuff.shape[-2:], device=self.device)
            pred_sem_seg[class_sts] += pred_stuff
            processed_results.append(
                {"sem_seg": pred_sem_seg, "instances": result_instance})
