...
def weighted_dice_loss(
    ...
    if mode == "thing":
        ...
        prediction = prediction.reshape(-1,
                                        weighted_num, h, w)[index_mask, ...]
        target_seg = target_seg.unsqueeze(2).expand(
            n, instance_num, weighted_num, h, w)
        target_seg = target_seg.reshape(-1,
                                        weighted_num, h, w)[index_mask, ...]
        weighted_val = weighted_val.reshape(-1, weighted_num)[index_mask, ...]
        weighted_val = weighted_val / \
            torch.clamp(weighted_val.sum(dim=-1, keepdim=True), min=eps)
    ...
    loss = 1 - 2 * (target_seg * prediction).sum(dim=-1) / \
        torch.clamp(loss_part, min=eps)
    # normalize the loss
