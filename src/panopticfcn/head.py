...
class PositionHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.position_head = SingleHead(in_channel+2 if coord else in_channel,
                                        ...
                                        norm=norm,
                                        name='position_head')
        self.out_inst = Conv2d(conv_dims, thing_classes,
                               kernel_size=3, padding=1)
        self.out_sem = Conv2d(conv_dims, stuff_classes,
                              kernel_size=3, padding=1)
...
class ThingGenerator(nn.Module):
    ...
    def forward(self, x, feat_shape, idx_feat, idx_shape, pred_cate=None, pred_score=None):
        ...
        if idx_shape > 0:
            ...
            if not self.training:
                meta_weight, pred_cate, pred_score = self.kernel_fusion(
                    meta_weight, pred_cate, pred_score)
    ...
    def kernel_fusion(self, meta_weight, pred_cate, pred_score):
        ...
        similarity = self.cal_similarity(
            meta_weight, meta_weight, sim_type=self.sim_type)
        ...
        label_matrix = (label_matrix[keep_matrix]
                        & cum_matrix[keep_matrix]).float()
...
class StuffGenerator(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.embed_extractor = Conv2d(
            input_channels, self.conv_dims, kernel_size=1)
        for layer in [self.embed_extractor]:
            nn.init.normal_(layer.weight, mean=0, std=0.01)
            if layer.bias is not None:
                nn.init.constant_(layer.bias, 0)
    ...
    def kernel_fusion(self, meta_weight, pred_cate, pred_score):
        ...
        meta_weight = meta_weight.squeeze(0)
        cate_matrix, uniq_matrix = pred_cate.unsqueeze(
            0), unique_cate.unsqueeze(1)
        label_matrix = (cate_matrix == uniq_matrix).float()
        label_norm = label_matrix.sum(dim=1, keepdim=True)
        meta_weight = torch.mm(label_matrix, meta_weight) / label_norm
        pred_score = torch.mm(
            label_matrix, pred_score.unsqueeze(-1)) / label_norm
