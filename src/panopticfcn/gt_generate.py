...
class GenerateGT(object):
    ...
    def assign_multi_scale(self, gt_instances, gt_semantics, image_shape, inst_outsize, feat_shape):
        ...
        h, w = gt_sem.shape[-2:]
        new_h, new_w = int(h / self.common_stride +
                           0.5), int(w / self.common_stride + 0.5)
        gt_sem = F.interpolate(gt_sem.unsqueeze(0), size=(new_h, new_w),
                               mode='bilinear', align_corners=False)[0]
        gt_semantics = torch.zeros(
            self.stuff_classes, *inst_outsize, device=self.device)
        ...
        if hasattr(gt_instances, 'gt_masks'):
            ...
            for layer_idx in range(len(feat_shape)):
                gt_center_per = torch.zeros(
                    self.thing_classes, *feat_shape[layer_idx], device=self.device)
                gt_inst_per = self.ignore_val * \
                    torch.ones(self.tensor_dim, *inst_outsize,
                               device=self.device)
                gt_index_per = torch.zeros(self.tensor_dim, device=self.device)
                index_mask_per = torch.zeros(
                    self.tensor_dim, device=self.device)
    ...
    def _label_assignment(self, gt_class_raw, gt_masks_raw, gt_centers_raw, gt_boxes_raw, gt_areas_raw, inst_scale, image_shape, inst_outsize, feat_shape):
        ...
        # init gt tensors
        gt_scoremap = torch.zeros(
            self.thing_classes, *feat_shape, device=self.device)
        gt_instance = self.ignore_val * \
            torch.ones(self.tensor_dim, *inst_outsize, device=self.device)
        ...
        box_rescale = [feat_shape[-2]/image_shape[-2],
                       feat_shape[-1]/image_shape[-1]]
        center_rescale = [feat_shape[-2]/inst_outsize[-2],
                          feat_shape[-1]/inst_outsize[-1]]
        gt_assign_mask = ((gt_areas_raw >= inst_scale[0]) & (
            gt_areas_raw <= inst_scale[1]))
        ...
        gt_instance[:num_inst, :bit_masks.shape[1],
                    :bit_masks.shape[2]] = bit_masks
        gt_index[:num_inst] = centers_int[..., 1] * \
            feat_shape[1] + centers_int[..., 0]
    ...
    def _sem_assign(self, gt_semantic, feat_shape):
        ...
        gt_scoremap = F.interpolate(gt_semantic.unsqueeze(
            0), size=feat_shape, mode='bilinear', align_corners=False)[0]
        gt_scoremap = gt_scoremap.clamp(max=1.0)
        gt_scoremap[gt_scoremap < 0.5] = 0.0
        gt_assign_mask = gt_scoremap.reshape(
            self.stuff_classes, -1).sum(dim=-1) > 0
    ...
    def generate(self, batched_input, images, features, inst_feat, test_only=False):
        ...
        gt_centers, gt_insts, gt_indexes, index_masks, gt_classes = [
            [] for _ in range(5)]
        gt_scores_sem, gt_labels_sem, gt_mask_sem, gt_index_sem = [
            [] for _ in range(4)]
    ...
    def draw_gaussian(fmap, center, radius, k=1, sigma_factor=6, device=None):
        ...
        """
        diameter = 2 * radius + 1
        gaussian = GenerateGT.gaussian2D(
            (radius, radius), sigma=diameter / sigma_factor)
